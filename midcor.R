...
correct<-function(fname,samb=1,samf=3,cndb=4,cndf=9){#fname is the name of file with raw data;
...
# Sum the data for various injections from the same plate:
  lst=suminj(gcms,nln,coln,samb,samf); gcms=lst[[1]]; nln=lst[[2]];
# normalization
         gcmsn<-norm(gcms,ef);
# mass fractions
...
# statistics in row data     
  lst=stat(tmp,nln,nfrg,cndb,cndf); gcmsn2=lst[[1]]; gcmsn2$V2=NULL; 
